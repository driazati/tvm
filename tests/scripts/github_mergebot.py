...
class PR:
    ...
    def merge_if_passed_checks(self):
        failed_ci_jobs = [
            job for job in self.ci_jobs() if job["status"] in {"FAILED", "ERROR", "FAILURE"}
        ]
        all_ci_passed = False
        has_one_approval = False
        if len(failed_ci_jobs) > 0:
            failed_jobs_msg = "\n".join(
                [f" * [{job['name']}]({job['url']})" for job in failed_ci_jobs]
            )
            self.comment(f"Cannot merge, these CI jobs failed on {self.head_oid()}:\n{failed_jobs_msg}")
            return
        else:
            all_ci_passed = True
    ...
    if pr.merge_requested():
        try:
            ...
            if not args.dry_run:
                ...
                pr.comment(f"Failed to process merge request in {args.run_url}\n\n<details>\n\n```\n{msg}\n```\n\n</details>")
            raise e
    else:
